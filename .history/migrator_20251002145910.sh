...
# Функция проверки Applied ли миграция по имени (ИСПРАВЛЕНА)
...
# Функция проверки Applied ли миграция по хешу (ИСПРАВЛЕНА)
...
get_migration_info_by_hash() {
    local file_hash="$1"
    run_sql_c "SELECT migration_name FROM migrations WHERE file_hash = '$file_hash' LIMIT 1;" | head -1
}
...
apply_migrations() {
    ...
    for sql_file in "$MIGRATIONS_DIR"/*.sql; do
        ...
        if is_migration_applied_by_name "$migration_name"; then
            echo "⏭️  $migration_name - уже применена"
            ((skipped_count++))
        ...
        # Проверяем, было ли уже применено такое же содержимое под другим именем
            ...
            if run_sql_c "$update_sql" > /dev/null; then
                echo " $migration_name - имя обновлено"
                ((renamed_count++))
            else
...
show_migration_status() {
    ...
    applied_migrations=$(run_sql_c "SELECT migration_name FROM migrations ORDER BY applied_at;")
